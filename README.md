...
[![NPM version](https://img.shields.io/npm/v/hari_test.svg)](https://npmjs.org/package/hari_test)
...
async function main() {
  const pet = await hariTest.pets.retrieve('123555');
...
async function main() {
  const pet: HariTest.Pet = await hariTest.pets.retrieve('123555');
}
...
```ts
async function main() {
  const pet = await hariTest.pets.retrieve('123555').catch((err) => {
...
await hariTest.pets.retrieve('123555', {
  maxRetries: 5,
});
```
...
const response = await hariTest.pets.retrieve('123555').asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: pet, response: raw } = await hariTest.pets.retrieve('123555').withResponse();
...
- Deno v1.28.0 or higher, using `import HariTest from "npm:hari_test"`.
- Bun 1.0 or later.
- Cloudflare Workers.
- Vercel Edge Runtime.
