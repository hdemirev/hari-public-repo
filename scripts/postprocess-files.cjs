...
const pkgImportPath = process.env['PKG_IMPORT_PATH'] ?? 'hari-new-test-org-project-2/'
...
async function postprocess() {
  for await (const file of walk(path.resolve(__dirname, '..', 'dist'))) {
    ...
    if (file.endsWith('.d.ts')) {
      ...
      // if we have `import { type Readable } from 'hari-new-test-org-project-2/_shims/index'`,
